...
- `src/data-client.js`: cliente de solo lectura para salud y datos agregados.
- `api/health.js`: comprobación segura de las vistas públicas desde Vercel.
- `api/flows.js`: agregación server-side del eje Flujos y recogidas a partir de `v_public_flujos_*`.
- `src/demo-data.js`: muestra local mínima; se usa únicamente si no se está ejecutando en Vercel.
...
Las funciones usan solo esa clave publicable y las vistas `analytics.v_public_flujos_*`; no descargan filas crudas al navegador. La `SUPABASE_SERVICE_ROLE_KEY` se reserva para el pipeline local de carga.
...
Abrir `http://localhost:4173`. Sin el entorno de Vercel, el visor muestra una demostración acotada y la conexión se indicará como pendiente de configurar; no sustituye los datos de Supabase.
